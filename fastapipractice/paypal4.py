...
if not Path(EXCEL_FILE).exists():
    df = pd.DataFrame(columns=[
        "TransactionID", "StudentName", "RollNo", "Email", "Phone", "Amount", "Status", "CreatedAt"
    ])
    df.to_excel(EXCEL_FILE, index=False)

def save_transaction(txnid, student_name, roll_no, email, phone, amount, status):
    df = pd.read_excel(EXCEL_FILE)
    if txnid in df["TransactionID"].values:
        df.loc[df["TransactionID"] == txnid, "Status"] = status
    else:
...
def create_payment(
    ...
    # Save as PENDING in Excel
    save_transaction(txnid, student_name, roll_no, email, phone, amount, "PENDING")
...
def payment_success(txnid: str, token: str, PayerID: str):
    ...
    if payment:
        ...
        db.commit()
        # Update Excel
        save_transaction(txnid, payment.student_name, payment.roll_no, payment.email, payment.phone, payment.amount, "SUCCESS")
    db.close()
...
def payment_cancel(txnid: str):
    ...
    if payment:
        ...
        db.commit()
        # Update Excel
        save_transaction(txnid, payment.student_name, payment.roll_no, payment.email, payment.phone, payment.amount, "CANCELLED")
    db.close()
